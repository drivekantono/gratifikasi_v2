               ...
               <select name="periode" class="form-control" >
                  ...
                  @else
                      @foreach($data as $key => $item)
                        @if($new === $item->tahun1."-".$item->tahun2)
                          <option value="{{$item->tahun1."-".$item->tahun2 }}"  selected="">{{$item->tahun1."-".$item->tahun2 }}</option>
                        @else
                          <option value="{{$item->tahun1."-".$item->tahun2 }}" >{{$item->tahun1."-".$item->tahun2 }}</option>
                        @endif
                      @endforeach
                  @endif
                </select>
      ...
      <div class="row">
        <div class="col-lg-12 col-md-12">
          <div class="left-side">
            <div class="post">
              <div class="post-desc">
                <div class="post-date mb-2">Data Kosong</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="row">
        <div class="col-lg-12 col-md-12">
          <div class="left-side">
            <div class="post">
              <div class="post-desc">
                <div class="post-date mb-2">Renstra Strategis Tahun {{ $item->tahun1 }} - {{ $item->tahun2 }}</span>
                </div>
                ...
                <span><a href="{{asset('uploads/renstra_strategis/'.$item->file)}}">Download</a></span>
              </div>
            </div>
          </div>
        </div>      
