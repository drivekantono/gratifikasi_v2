...
@section('content')
    <div class="box">
            <div class="box-header">
              <h3 class="box-title">Dataaa Link Informasi</h3>
            </div>
            <div class="box-header">
              <a href="{{ route('admin.link_informasi.create') }}" class="btn btn-primary" style="margin-bottom: 15px;"><i class="fa fa-plus"></i>Tambah Data</a>  
              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                @foreach ($items as $item)
                  <tr>
                    ...
                    <td><a href="{{ asset('/uploads/link_informasi/'.$item->file) }}">{{ $item->file }}</a></td>
                    <td>
                      <a href="{!! route('admin.link_informasi.edit', $item->id) !!}"><button class="btn btn-success"><i class="fa fa-edit"></i></button></a>
                    </td>
                    <td>
                      <form action="{{ route('admin.link_informasi.destroy', $item->id) }}" method="GET" style="display:inline-block;">
                        <button title="Delete" class="btn btn-danger js-submit-confirm" type="submit"><i class="fa fa-trash-o"></i></button>
                      </form>
                    </td>
                    <td>
                      {{--  <form action="{{ route('admin.link_informasi.show', $item->id) }}" method="GET" style="display:inline-block;">  --}}
...
<script>
  ...
  function load_data(id){
    var url_php = '{{ route('admin.link_informasi.show', 'id') }}';
    ...
    $.ajax({
      ...
      dataType: 'json', // added data type
        success: function(res) {
            ...
            $('#images').attr('src', '/uploads/link_informasi/'+res.images);
        }
    })
