        ...
        <div class="col-md-12">
            ...
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="box-title">Edit Link Layanan</h3>
                </div>
                @if (count($errors) > 0)
              ...
              @endif
                <!-- /.box-header -->
                <!-- form start -->
                <form role="form" action="{{ route('admin.link_layanan.update', $items->id) }}" method="POST" enctype="multipart/form-data">
                    {{ csrf_field() }}
                    <div class="box-body">
                        @if (count($errors) > 0)
                            <div class="alert alert-danger">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="link" name="link" placeholder="Enter Title" value=" {{ old('link') ? old('link') : $items->link }}">
                        </div>
                        <div class="form-group">
                            <label for="image">Logo Aplikasi</label>
                            <input type="file" id="image" name="image">
                            {{--  <img src="{{ asset('uploads/link_layanan/'.$items->images) }}" style="max-width:50px;max-height:50px;float:left;">  --}}
                            <p class="help-block">Masukan Gambar</p>
                        </div>
                        <td>
                            {{-- <input type="checkbox" name="hapus" value="1">Hapus Gambar Sebelumnya<br> --}}
                            <img src="{{ asset('/uploads/link_layanan/'.$items->images) }}" height="30%" width="30%" />
                        </td>
                    </div>
                    <!-- /.box-body -->
