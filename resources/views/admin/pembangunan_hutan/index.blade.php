...
@section('content')
    <div class="box">
            <div class="box-header">
              <h3 class="box-title">Data Pembangunan Hutan</h3>
            </div>
            <div class="box-header">
              <a href="{{ route('admin.master_ph.create') }}" class="btn btn-primary" style="margin-bottom: 15px;"><i class="fa fa-plus"></i>Tambah Master</a>  
            </div>
            <!-- /.box-header -->
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                @foreach ($items as $item)
                  <tr>
                    ...
                    <td>
                      <img src="{{ asset('/uploads/pembangunan_hutan/'.$item->images) }}" height="50px" width="70px" />
                    </td>
                    <td>
                      <a href="{!! route('admin.master_ph.edit', $item->id) !!}"><button class="btn btn-success"><i class="fa fa-edit"></i></button></a>
                    </td>
                    <td>
                      <form action="{{ route('admin.master_ph.destroy', $item->id) }}" method="GET" style="display:inline-block;">
                        <button title="Delete" class="btn btn-danger js-submit-confirm" type="submit"><i class="fa fa-trash-o"></i></button>
                      </form>
                    </td>
                    <td>
                      {{-- <form action="{{ route('admin.master_ph.show', $item->id) }}" method="GET" style="display:inline-block;"> --}}
...
<script>
  ...
  function load_data(id){
    var url_php = '{{ route('admin.master_ph.show', 'id') }}';
    ...
    $.ajax({
      ...
      dataType: 'json', // added data type
        success: function(res) {
            ...
            $('#images').attr('src', '/uploads/pembangunan_hutan/'+res.images);
        }
    })
