...
@section('content')
    <div class="box">
            ...
            <div class="box-header">
              <a href="{{ route('admin.agenda.create') }}" class="btn btn-primary" style="margin-bottom: 15px;"><i class="fa fa-plus"></i>Tambah Data</a>  
              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                @foreach ($items as $item)
                  <tr>
                    <td>{{ $no++ }}</td>
                    <td>{{ $item->title }}</td>
                    <td>{{ mb_substr(strip_tags($item->deskripsi) ,0, 100) }}...</td>
                    <td>{{ $item->tanggal }}</td>
                    <td>{{ $item->tanggal_selesai }}</td>
                    <td>{{ $item->lokasi }}</td>
                    <td>
                      <img src="{{ asset('/uploads/agenda/'.$item->images) }}" height="50px" width="70px" />
                    </td>
                    <td>
                      <a href="{!! route('admin.agenda.edit', $item->id) !!}"><button class="btn btn-success"><i class="fa fa-edit"></i></button></a>
                    </td>
                    <td>
                      <form action="{{ route('admin.agenda.destroy', $item->id) }}" method="GET" style="display:inline-block;">
                        <button title="Delete" class="btn btn-danger js-submit-confirm" type="submit"><i class="fa fa-trash-o"></i></button>
                      </form>
                    </td>
                    <td>
                      {{-- <form action="{{ route('admin.agenda.show', $item->id) }}" method="GET" style="display:inline-block;"> --}}
            ...
            <div class="box-body box-profile">
              ...
              <ul class="list-group list-group-unbordered">
                <li class="list-group-item">
                  ...
                  <p class="text-muted text-center" id="tanggal_selesai"></p>
                </li>
                <li class="list-group-item">
...
<script>
  ...
  function load_data(id){
    var url_php = '{{ route('admin.agenda.show', 'id') }}';
    ...
    $.ajax({
      ...
      dataType: 'json', // added data type
        success: function(res) {
            ...
            document.getElementById("tanggal").innerHTML = res.tanggal;
            document.getElementById("tanggal_selesai").innerHTML = res.tanggal_selesai;
            document.getElementById("lokasi").innerHTML = res.lokasi;
            $('#images').attr('src', '/uploads/agenda/'+res.images);
        }
    })
