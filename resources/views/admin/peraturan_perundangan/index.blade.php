...
@section('content')
    <div class="box">
            <div class="box-header">
              <h3 class="box-title">Data Peraturan Perundangan</h3>
            </div>
            <div class="box-header">
              <a href="{{ route('admin.peraturan_perundangan.create') }}" class="btn btn-primary" style="margin-bottom: 15px;"><i class="fa fa-plus"></i>Tambah Master</a>  
            </div>
            <!-- /.box-header -->
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                @foreach ($items as $item)
                  <tr>
                    ...
                    <td>
                      <a href="{{ asset('/uploads/peraturan_perundangan/'.$item->images) }}">{{ $item->images }}</a>
                    </td>
                    <td>
                      <a href="{!! route('admin.peraturan_perundangan.edit', $item->id) !!}"><button class="btn btn-success"><i class="fa fa-edit"></i></button></a>
                    </td>
                    <td>
                      <form action="{{ route('admin.peraturan_perundangan.destroy', $item->id) }}" method="GET" style="display:inline-block;">
                        <button title="Delete" class="btn btn-danger js-submit-confirm" type="submit"><i class="fa fa-trash-o"></i></button>
                      </form>
                    </td>
                    <td>
                      {{-- <form action="{{ route('admin.peraturan_perundangan.show', $item->id) }}" method="GET" style="display:inline-block;"> --}}
...
<script>
  ...
  function load_data(id){
    var url_php = '{{ route('admin.peraturan_perundangan.show', 'id') }}';
    ...
    $.ajax({
      ...
      dataType: 'json', // added data type
        success: function(res) {
            ...
            $('#images').attr('src', '/uploads/peraturan_perundangan/'+res.images);
        }
    })
