...
@section('content')
    <div class="box">
            <div class="box-header">
              <h3 class="box-title">List Data Statistika</h3>
            </div>
            <div class="box-header">
              <a href="{{ route('admin.data_statistika.create') }}" class="btn btn-primary" style="margin-bottom: 15px;"><i class="fa fa-plus"></i>Tambah Data</a>  
              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                @foreach ($items as $item)
                  <tr>
                    ...
                    <td>{{ $item->title }}</td>
                    <td>{{ $item->tahun }}</td>
                    <td><a href="{{ asset('/uploads/data_statistika/'.$item->file) }}">{{ $item->file }}</a></td>
                    <td>
                      <a href="{!! route('admin.data_statistika.edit', $item->id) !!}"><button class="btn btn-success"><i class="fa fa-edit"></i></button></a>
                    </td>
                    <td>
                      <form action="{{ route('admin.data_statistika.destroy', $item->id) }}" method="GET" style="display:inline-block;">
                        <button title="Delete" class="btn btn-danger js-submit-confirm" type="submit"><i class="fa fa-trash-o"></i></button>
                      </form>
                    </td>
                    <td>
                      {{--  <form action="{{ route('admin.data_statistika.show', $item->id) }}" method="GET" style="display:inline-block;">  --}}
...
<script>
  ...
  function load_data(id){
    var url_php = '{{ route('admin.data_statistika.show', 'id') }}';
    ...
    $.ajax({
      ...
      dataType: 'json', // added data type
        success: function(res) {
            ...
            $('#images').attr('src', '/uploads/data_statistika/'+res.images);
        }
    })
