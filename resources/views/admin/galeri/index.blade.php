...
@section('content')
    <div class="box">
            ...
            <div class="box-header">
              <a href="{{ route('admin.galeri.create') }}" class="btn btn-primary" style="margin-bottom: 15px;"><i class="fa fa-plus"></i>Tambah Data</a>
              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                @foreach ($items as $item)
                  <tr>
                    ...
                    <td>{{ $item->id_kategori == null ? "tidak ada kategori" : $item->kategoriGaleri->nama_kategori }}</td>
                    <td>{{ $item->status }}</td>
                    <td>
                      <img src="{{ asset('/uploads/galeri/'.$item->images) }}" height="50px" width="70px" />
                    </td>
                    <td>{{ $item->link_vidio }}</td>
                    <td>
                      <a href="{!! route('admin.galeri.edit', $item->id) !!}"><button class="btn btn-success"><i class="fa fa-edit"></i></button></a>
                    </td>
                    <td>
                      <form action="{{ route('admin.galeri.destroy', $item->id) }}" method="GET" style="display:inline-block;">
                        <button title="Delete" class="btn btn-danger js-submit-confirm" type="submit"><i class="fa fa-trash-o"></i></button>
                      </form>
                    </td>
                    <td>
                      {{-- <form action="{{ route('admin.galeri.show', $item->id) }}" method="GET" style="display:inline-block;"> --}}
            ...
            <div class="box-body box-profile">
              ...
              <ul class="list-group list-group-unbordered">
                <li class="list-group-item">
                  <p class="text-muted text-center" id="status"></p>
                </li>
                <li class="list-group-item">
                  <a><p class="text-muted text-center" id="link_vidio"></p></a>
                </li>
              </ul>
            </div>
            <!-- /.box-body -->
...
<script>
  ...
  function load_data(id){
    var url_php = '{{ route('admin.galeri.show', 'id') }}';
    ...
    $.ajax({
      ...
      dataType: 'json', // added data type
        success: function(res) {
            ...
            document.getElementById("link_vidio").innerHTML = res.link_vidio;
            $('#images').attr('src', '/uploads/galeri/'+res.images);
        }
    })  
  }
</script>
