...
@section('content')
    <div class="box">
            ...
            <div class="box-header">
              <a href="{{ route('admin.berita.create') }}" class="btn btn-primary" style="margin-bottom: 15px;"><i class="fa fa-plus"></i>Tambah Data</a>  
              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                @foreach ($items as $item)
                  <tr>
                    ...
                    <td>{{ $item->title }}</td>
                    <td>{{ $item->tag }}</td>
                    <td>{{ $item->KategoriBerita->kategori_berita }}</td>
                    <td>{!! mb_substr($item->deskripsi ,0, 100) !!}...</td>
                    <td>
                      <img src="{{ asset('/uploads/berita/'.$item->images) }}" height="50px" width="70px" />
                    </td>
                    <td>
                      <a href="{!! route('admin.berita.edit', $item->id) !!}"><button class="btn btn-success"><i class="fa fa-edit"></i></button></a>
                    </td>
                    <td>
                      <form action="{{ route('admin.berita.destroy', $item->id) }}" method="GET" style="display:inline-block;">
                        <button title="Delete" class="btn btn-danger js-submit-confirm" type="submit"><i class="fa fa-trash-o"></i></button>
                      </form>
                    </td>
                    <td>
                      {{-- <form action="{{ route('admin.berita.show', $item->id) }}" method="GET" style="display:inline-block;"> --}}
...
<script>
  ...
  function load_data(id){
    var url_php = '{{ route('admin.berita.show', 'id') }}';
    ...
    $.ajax({
      ...
      dataType: 'json', // added data type
        success: function(res) {
            ...
            $('#images').attr('src', '/uploads/berita/'+res.images);
        }
    })
