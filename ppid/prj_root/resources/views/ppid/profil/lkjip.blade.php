    ...
    <div class="container">
      <div class="row align-items-center" style="margin-top: -130px;">
        ...
        <div class="col-lg-6 col-md-12 order-lg-1 md-mt-5">
          <h1 class=" text-white animated bounceInLeft delay-2 duration-3">LAPORAN KINERJA INSTANSI PEMERINTAH</h1>
        </div>        
      </div>
    </div>
   </div>
  ...
  <div class="container">
    ...
    <div class="col-lg-12 col-md-12 md-mt-5">
        ...
        <div class="subscribe-form sm-mt-2">
          <form id="mc-form" class="group" action="{{ route('frontend.lkjip.bytahun') }}" method="POST">
            {{ csrf_field() }}
    ...
    <div class="row">
      <div class="col-lg-12 col-md-12">
        <div class="left-side">
          <div class="post">
            <div class="post-desc">
              <div class="post-date mb-2">LKJIP {{ $item->tahun}}</span>
              </div>
              <p class="lead">{!! $item->title !!}
                </object> 
                <a href="{{ asset('/uploads/lkjip/'.$item->file) }}">Download</a>
            </div>
          </div>
        </div>
      </div>      
